...
----------------------
  ...
  -- 如果沒有下一個state，則找到終止，否則會繼續往 cd ≡ 0 的 case 前進
  Finite-State-Termination-With-Countdown  St-Fin has-next st₀-initial (suc cd) _ stₘ sum-eql st₀↦ᵐstₘ with has-next stₘ
  ... | _ because ofⁿ ¬p = stₘ , [n]→* st₀↦ᵐstₘ , ¬p
  ... | _ because ofʸ (stₘ₊₁ , stₘ↦stₘ₊₁) with cd-1 {cd} sum-eql
  ... | sum-eql with st₀↦ᵐstₘ ++↦ⁿ (stₘ↦stₘ₊₁ ∷ ◾)
  ... | st₀↦stₘ₊₁ =  Finite-State-Termination-With-Countdown St-Fin has-next st₀-initial cd _ stₘ₊₁ sum-eql st₀↦stₘ₊₁
  
  -- 當 cd ≡ 0 ，得到 m ≡ N ，從 stₙ→⊥
  Finite-State-Termination-With-Countdown St-Fin has-next st₀-initial 0 m stₘ refl st₀↦ᵐstₘ
  ...
  Finite-State-Termination : ∀ {N st₀}
    ...
    → is-initial st₀
    → ∃[ stₙ ] (st₀ ↦* stₙ × is-stuck stₙ)
  Finite-State-Termination {N} {st₀}  St-Fin has-next st₀-initial = Finite-State-Termination-With-Countdown St-Fin has-next st₀-initial N 0 st₀  (+-identityʳ N) ◾
