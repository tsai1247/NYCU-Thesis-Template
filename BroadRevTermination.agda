...
-----------------------------------------
  ...
  Finite-Reachable-State-Termination-CountDown {N} {st₀} St-Fin has-next st₀-initial (suc cd) m stₘ sum-eql st₀↦ᵐstₘ with has-next stₘ
  ... | .false because ofⁿ ¬p = stₘ , [n]→* st₀↦ᵐstₘ , ¬p
  ... | .true because ofʸ (stₘ₊₁ , stₘ↦stₘ₊₁) with cd-1 {cd} sum-eql
  ... | sum-eql with st₀↦ᵐstₘ ++↦ⁿ (stₘ↦stₘ₊₁ ∷ ◾)
  ... | st₀↦stₘ₊₁ = Finite-Reachable-State-Termination-CountDown  {N} {st₀} St-Fin has-next st₀-initial cd (m + 1) stₘ₊₁ sum-eql st₀↦stₘ₊₁ 

  Finite-Reachable-State-Termination-CountDown {N} {st₀} St-Fin has-next st₀-initial 0 m stₘ refl st₀↦ᵐstₘ with Finite-State-Termination-At-N {N} {st₀} St-Fin st₀-initial  (stₘ  , st₀↦ᵐstₘ)
  ... | ()
  ...
  Finite-Reachable-State-Termination : ∀ {N st₀}
    ...
    → is-initial st₀
    → ∃[ stₙ ] (st₀ ↦* stₙ × is-stuck stₙ)
  Finite-Reachable-State-Termination {N} {st₀} St-Fin has-next st₀-initial
    = Finite-Reachable-State-Termination-CountDown {N} {st₀} St-Fin has-next st₀-initial N 0 st₀ (+-identityʳ N) ◾

  Full-Finite-Reachable-State-Termination : ∀ {N st₀}
    → (St-Fin : ∃[ m ] ∃[ stₘ ] (st₀ ↦[ m ] stₘ) ⤖ Fin N)
    → (has-next : ∀ (st : State) → Dec (∃[ st' ] (st ↦ st')))
    → ∃[ stₙ ] (st₀ ↦* stₙ × is-stuck stₙ)
  Full-Finite-Reachable-State-Termination {N} {st₀} St-Fin has-next = {!!}
